...
export default class QueueUtil {
    ...
    static async publishMessageToExchange (exchange: Exchanges, exchangeType: ExchangeTypes, routingKey: string, exchangeOptions: Options.AssertExchange, publishOptions: Options.Publish, message?: any): Promise<void> {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                channel.publish(exchange, routingKey, Buffer.from(message ? JSON.stringify(message) : ""), publishOptions);
                setTimeout(async () => {
